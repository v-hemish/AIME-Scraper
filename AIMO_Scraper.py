...
def get_problems(url):
    ...
    for i in range(1, 16):  # Assuming there are 15 problems labeled Problem_1 to Problem_15
        problem_id = f"Problem_{i}"
        header = soup.find('span', id=problem_id)
        if header:
            # Get all subsequent <p> tags until another tag is encountered
            question_parts = []
            current_element = header.parent.find_next_sibling()
            while current_element and current_element.name == 'p':
                # Replace image tags with their alt text
                for img in current_element.find_all('img'):
                    alt_text = img.get('alt', '[Image]')
                    img.replace_with(alt_text)
                question_parts.append(current_element.get_text(" ", strip=True))
                current_element = current_element.find_next_sibling()
            if question_parts:
                problem_text = ' '.join(question_parts)
                problems_data.append(problem_text)
                print(f"Extracted {problem_id}: {problem_text[:60]}...")
            else:
                problems_data.append('No question found')
                print(f"Extracted {problem_id}: No question found...")
        else:
            problems_data.append('No question found')
            print(f"Extracted {problem_id}: No question found...")
